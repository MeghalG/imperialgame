...
class LoginApp extends React.Component{
    ...
    async play() {
        await this.doStuff();
        await database.ref('games/'+this.context.game+'/timer/lastMove').set(this.state.time-this.state.timer.pause+this.state.timer.lastMove);
        ...
        if (this.state.timer.timed) {
            ...
            if (this.state.timer.timed && this.state.validNames.includes(this.context.name) && this.state.myTurn) {
                ...
                if (this.state.timer.pause) {
                    ...
                    banked = Math.min(Math.max(this.state.timer.banked[this.context.name]*1000-this.state.timer.pause+this.state.timer.increment*1000+this.state.timer.lastMove, 0), this.state.timer.banked[this.context.name]*1000);
                }
                else {
                    inc = Math.max(this.state.timer.increment*1000-this.state.time+this.state.timer.lastMove, 0);
                    let ti = this.state.timer.banked[this.context.name]*1000-this.state.time+this.state.timer.increment*1000+this.state.timer.lastMove;
                ...
                t.push(<span>{this.msToTime(inc)} + {this.msToTime(banked)}</span>)
            }
            if (this.state.timer.pause) {
                t.push(<Button type="link" onClick={() => this.play()}> <i class="fas fa-play"></i> </Button>);
            }
            else {
                t.push(<Button type="link" onClick={() => this.pause()}> <i class="fas fa-pause"></i> </Button>);
            }
    ...
    buildExitGame() {
        ...
        if (this.context.game) {
            ...
            t.push(<Button type="link" onClick={() => this.exitGame()}> Exit Game </Button>);
        }
    ...
    buildComponent() {
        ...
        if (!this.context.name) {
            t.push( <div style={{fontSize: "18px", textAlign:"right"}}> 
                    {this.buildTimer()}
                    <Input placeholder="Name" allowClear={true} style={{background:"black"}} onPressEnter={(e) => this.handleEnter(e)} style={{width:150}} ></Input> &nbsp;
                    {this.buildExitGame()}
                </div>
            );
        }
            ...
            t.push( <div style={{fontSize: "18px", textAlign:"right"}}>  
                        {this.buildTimer()}
                        {this.context.name} &nbsp;
                        <Button type="link" onClick={() => this.logout()}> <i class="fas fa-sign-out-alt"></i> </Button>
                        {this.buildExitGame()}
                        &nbsp; &nbsp; &nbsp;
                    </div>
            )
