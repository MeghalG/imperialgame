...
buildComponents = () => {
    ...
    // if in players only
    let pc = <Panel header={"View your details"} key="player" >
            <div style={{marginLeft:9}}>
                <PlayerCard w="100%" h='100%' bg="black" p="15px 20px 0px 20px" br={0.001} ml={9} b="#707070"
                            player={''} countryColors={countryColors} info={this.clean(this.state.gameState.playerInfo[this.context.name])} countryInfos={this.state.gameState.countryInfo}/>
            </div>
            </Panel>
    let cc = <Panel header={"View " +country+ " details"} key="country" >
                    <CountryCard w="100%" h="100%" bg="black" country={''} p="15px 20px 0px 20px" br={0.001} ml={9} b={colors[country]}
                                color = {colors[country]} darkColor={colors[country]} info={this.clean(this.state.gameState.countryInfo[country])}/>
                </Panel>
	switch (this.state.gameState.mode) {
			...
			return (
				...
				<Collapse ghost style={{marginLeft:-18, marginBottom:-20, marginTop:-10 }}>
                    ...
                    <Panel header={"View bids in order"} key="1">
                        <div style={{marginLeft:9}}>
                        <Card style={{ marginLeft:"auto", marginRight:"auto", lineHeight:"0.8", borderColor:colors[country], backgroundColor:"black"}} bodyStyle={{padding: "15px 20px 0px 20px"}}>
                            {t}
                        </Card>
                        </div>
                    </Panel>
        ...
        case "vote":
			...
			if (this.state.gameState.mode == "proposal-opp") {
				pr = <Alert style={{marginBottom:10, width:"100%"}} message={this.state.gameState.history[this.state.gameState.history.length-1]} type="info"/> 
			}
			return (
				<div>
                    {pr}
