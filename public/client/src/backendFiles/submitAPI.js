...
async function finalizeSubmit(gameState, gameID, context) {
	...
	await database.ref('games/'+gameID).set(gameState, async (error) => {
		if (error) {
		  console.log("failed to write game state")
		} else {
...
async function adjustTime(player, gameState, t) {
	...
	console.log(time)
	let ti = gameState.playerInfo[player].banked*1000-time+gameState.timer.increment*1000+gameState.timer.lastMove;
	console.log(ti, gameState.playerInfo[player].banked, time, gameState.timer.increment, gameState.timer.lastMove);
	if (ti<0) {
		...
		gameState.playerInfo[player].banked=
			Math.min(Math.floor(gameState.playerInfo[player].banked - time/1000 + gameState.timer.lastMove/1000) + gameState.timer.increment, gameState.playerInfo[player].banked);
		console.log(gameState.playerInfo[player].banked)
	}
}
...
async function submitBuy(context) {
	...
	for (let i = 0; i<Object.keys(gameState.playerInfo).length-1; i++) {
		let index = ((players.indexOf(context.name))-1-i+Object.keys(gameState.playerInfo).length)%Object.keys(gameState.playerInfo).length;
	...
	gameState.history.push(context.name + " bought the " +context.buyCountry+ " " +context.buyStock+ " returning the " +context.buyCountry+ " " +context.returnStock+ ".");
	gameState.undo=context.name;
	console.log(gameState.swissSet);
...
async function submitVote(context) {
	...
	if (gameState.voting[proposal].votes>threshold) {
		gameState.history.push(context.name+ " has voted. " +gameState.voting['proposal 1'].voters.join(', ')+  " voted for the leader's proposal and " +gameState.voting['proposal 2'].voters.join(', ')+ " voted against.");
...
async function makeHistory(gameState, context) {
	...
	switch (context.wheelSpot) {
		...
		case "R-Produce":
			...
			return country+ " " +context.wheelSpot+ "s armies in " +(context.armyProduce || []).join(', ')+ " and fleets in " +(context.fleetProduce || []).join(', ') +".";
		case "Taxation":
			let taxInfo = await helper.getTaxInfo(gameState.countryInfo, gameState.playerInfo, country);
			let s = country+ " taxes for " +taxInfo.points+ " points, and $" +taxInfo.money+ " into its treasury. Greatness is distributed ";
...
async function executeProposal(gameState, context) {
			...
			for (let army of sortedArmyMan) {
				...
				if (split[0]=="blow") {
					if (gameState.countryInfo[split[2]].factories.includes(army[1])) {
						gameState.countryInfo[split[2]].factories.splice(gameState.countryInfo[split[2]].factories.indexOf(army[1]), 1)
					}
					continue;
				}
	...
	let investorPassed = await helper.investorPassed(gameState.countryInfo[country].wheelSpot, context.wheelSpot, context);
	if (investorPassed) {
	...
	if (gameState.countryInfo[country].wheelSpot!='center') {
		diff = (wheel.indexOf(context.wheelSpot)-wheel.indexOf(gameState.countryInfo[country].wheelSpot)+wheel.length)%wheel.length;
	}
	if (diff>3) {
