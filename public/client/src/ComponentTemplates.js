...
class ImportComponent extends React.Component {
	...
	makeOptions = () => {
		...
		for (let key in this.state.options) {
			...
			disKeyOpt.push(
				<Option key={key} value={key} disabled={count === this.state.limits[key]}>
					{' '}
					{key}{' '}
				</Option>
			);
		...
		for (let i = 0; i < this.state.labels.length; i++) {
			let lab = (
				<label style={{ whiteSpace: 'nowrap', marginLeft: 10 }}>{this.state.labels[i]}: &nbsp; &nbsp; &nbsp; </label>
			);
			let sel = null;
	...
	async getStuff() {
		...
		await this.setState({
			...
			options: res.options,
			limits: res.limits,
			keyValues: new Array(res.labels.length).fill(''),
			values: new Array(res.labels.length).fill(''),
		});
		let valueOpt = {};
		for (let key in this.state.options) {
			valueOpt[key] = [];
...
class MultiOptionComponent extends React.Component {
	...
	makeOptions = () => {
		...
		for (let i = 0; i < this.state.choices.length; i++) {
			let t = [];
			let opt = this.state.choices[i][1];
			...
			let lab = (
				<label style={{ whiteSpace: 'nowrap', marginLeft: 10 }}>{this.state.choices[i][0]} &nbsp;&nbsp; ⟶ </label>
			);
			let sel = (
				<Select
					allowClear={true}
		...
		for (let i = 0; i < this.state.choices.length; i++) {
			...
			table.push(
				<Option key={i} value={opt}>
					{' '}
					{opt} {<span style={{ color: '#13a8a8' }}> {optCost} </span>}{' '}
				</Option>
			);
		}
		return table;
...
class RadioComponent extends React.Component {
	...
	makeOptions = () => {
		...
		for (let i = 0; i < this.state.choices.length; i++) {
			let opt = this.state.choices[i];
			table.push(
				<Radio value={i} style={{ marginLeft: 48, marginTop: 10, textIndent: -28, whiteSpace: 'normal' }}>
					{' '}
					{opt}{' '}
				</Radio>
			);
	...
	render() {
		...
		return (
			<div style={{ marginBottom: 30 }}>
				...
				<br />
				<Radio.Group onChange={(value) => this.sendValue(value)}>{this.makeOptions()}</Radio.Group>
			</div>
		);
	}
}
...
class CheckboxComponent extends React.Component {
	...
	makeOptions = () => {
		...
		for (let i = 0; i < this.state.items.length; i++) {
			let opt = this.state.items[i];
			table.push(
				<Checkbox value={opt} disabled={this.isDisabled({ opt })}>
					{' '}
					{opt} {this.state.type}{' '}
				</Checkbox>
			);
	...
	isDisabled = (obj) => {
		return this.state.checked.length > this.state.limit - 1 && this.state.checked.indexOf(obj.opt) === -1;
	};
...
class MessageComponent extends React.Component {
	...
	render() {
		return <div> {this.makeMessage()} </div>;
	}
}
...
class SimpleMessageComponent extends React.Component {
	...
	render() {
		return <div style={{ marginBottom: 10 }}>{this.state.message}</div>;
	}
}
...
class ActionComponent extends React.Component {
	...
	buildComponents = () => {
		...
		for (let i = 0; i < this.state.objects.length; i++) {
			...
			table.push(
				<Display
					component={this.state.components[this.state.objects[i]]}
					show={vis}
		...
		table.push(
			<Display
				component={Submit}
				show={this.state.visibleLayers[this.state.visibleLayers.length - 1] && this.state.submit}
				fn={this.submit.bind(this)}
				k={this.state.keys[this.state.visibleLayers.length - 1]}
	...
	render() {
		return <div className={this.state.className}>{this.buildComponents()}</div>;
	}
}
