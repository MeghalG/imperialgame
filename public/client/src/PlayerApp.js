  ...
  render() {
    return (
        ...
        <Space size="small" direction="vertical">
                    <PlayerCard player={this.state.playersOrdered[0]} countryColors={this.state.darkColors} info={clean(this.state.playerInfo[this.state.playersOrdered[0]])} countryInfos={this.state.countryInfo}/>
                    <PlayerCard player={this.state.playersOrdered[1]} countryColors={this.state.darkColors} info={clean(this.state.playerInfo[this.state.playersOrdered[1]])} countryInfos={this.state.countryInfo}/>
                    <PlayerCard player={this.state.playersOrdered[2]} countryColors={this.state.darkColors} info={clean(this.state.playerInfo[this.state.playersOrdered[2]])} countryInfos={this.state.countryInfo}/>
                    <PlayerCard player={this.state.playersOrdered[3]} countryColors={this.state.darkColors} info={clean(this.state.playerInfo[this.state.playersOrdered[3]])} countryInfos={this.state.countryInfo}/>
                    <PlayerCard player={this.state.playersOrdered[4]} countryColors={this.state.darkColors} info={clean(this.state.playerInfo[this.state.playersOrdered[4]])} countryInfos={this.state.countryInfo}/>
                    <PlayerCard player={this.state.playersOrdered[5]} countryColors={this.state.darkColors} info={clean(this.state.playerInfo[this.state.playersOrdered[5]])} countryInfos={this.state.countryInfo}/>
            </Space>
        </div>
    );
  }
...
class PlayerCard extends React.Component {
    ...
    formatStock(stock) {
        ...
        for (let i=0; i<s.length; i++) {
            for (let j=0; j<s[i].length; j++) {
                t.push(<mark style={{backgroundColor: this.props.countryColors[this.state.countries[i]], color: "white", borderRadius:2}}>{s[i][j]}</mark>)
                t.push(<span>&nbsp;</span>)
            }
        }
        return t;
    ...
    gov() {
        ...
        for (let country in this.props.countryInfos) {
            ...
            if ((this.props.countryInfos[country] || {}).gov=='democracy' && this.props.countryInfos[country].leadership[1]==this.props.player) {
                t.push(
    ...
    render() {
        if (this.props.player!=null) {
            return (
                <Card hoverable={true} size='small' style={{lineHeight:"0.8", minWidth: "100%", backgroundColor:"black", width:"11vw"}} 
