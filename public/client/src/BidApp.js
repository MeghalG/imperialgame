...
class BidApp extends React.Component{
    ...
    makeInput() {
        if (this.state.maxMoney!=-1) {
            return <InputNumber min={0} max={this.state.maxMoney} defaultValue={0.00} step={0.01} formatter={value => `$   ${value}`} parser={value => value.replace(/\$\s?|( *)/g, '')} onChange={(value) => this.record(value)} style={{width: 120}}/>
        }
    }
    ...
    render () {
        return (
            <div>
                <div style={{marginBottom: 30}}> You may bid up to ${this.state.maxMoney} on {this.state.country} in increments of cents. (It will round if you try to use more precision.) </div>
                <div style={{marginBottom: 30}}>
                    <label style={{paddingRight:'50px', whiteSpace:"nowrap", marginLeft:10}}> Bid: </label>
                    {this.makeInput()}<br />
                </div>
                <div style={{textAlign: "center"}}> <Button onClick={() => this.submit()} type="primary"> Submit </Button> </div>
            </div>
        );
    }
}
